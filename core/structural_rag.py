"""
Structural RAG Module - Document Tree Indexing
-------------------------------------------
...
try:
    from .godot_expert import GodotExpert
except ImportError:
    try:
        from godot_expert import GodotExpert
    except ImportError:
...
class StructuralIndex:
    ...
    def export_tree_json(self, output_path: str):
        ...
        print(f"[StructuralIndex] Tree exported to {output_path}")
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "files": len([n for n in self.index.values() if n.node_type == "File"]),
            "classes": len([n for n in self.index.values() if n.node_type == "Class"]),
            "functions": len([n for n in self.index.values() if n.node_type == "Function"]),
            "scenes": len([n for n in self.index.values() if n.node_type == "Scene"]),
            "scene_nodes": len([n for n in self.index.values() if n.node_type.startswith("SceneNode")])
        }


# Singleton instance
_instance: Optional[StructuralIndex] = None

def get_structural_index(project_root: str) -> StructuralIndex:
    """Get or create the singleton StructuralIndex instance."""
    global _instance
    if _instance is None or str(_instance.project_root) != project_root:
        _instance = StructuralIndex(project_root)
    return _instance


if __name__ == "__main__":
    # Test the StructuralIndex
    print("=== Testing StructuralIndex ===\n")
    ...
    try:
        # Create test GDScript file with class
...
""")
        ...
        test_scene = test_dir / "main.tscn"
        test_scene.write_text("""[gd_scene load_steps=2 format=2]
...
""")
        ...
        # Build index
        index = get_structural_index(str(test_dir))
        index.build_index()
        ...
        # Query for functions
        print("\n=== Query: All Functions ===")
        results = index.query_structure("function")
        for i, result in enumerate(results, 1):
            print(f"\n{i}. {result['path']}")
            print(f"   Type: {result['type']}")
            if result.get('structure_map'):
                print(f"   Children: {len(result['structure_map'])}")
        
        # Query for specific name
        print("\n=== Query: Find 'Player' ===")
        results = index.query_structure("class", target_name="Player")
        for i, result in enumerate(results, 1):
            print(f"\n{i}. {result['path']}")
            print(f"   Content preview: {result['content'][:150]}...")
        
        # Get context for path
        print("\n=== Context for player.gd ===")
        context = index.get_context_for_path("player.gd")
        if context:
            print(context[:500])
        
        # Export tree
        export_path = test_dir / "tree.json"
        index.export_tree_json(str(export_path))
        
        print("\n✅ StructuralIndex test complete!")
